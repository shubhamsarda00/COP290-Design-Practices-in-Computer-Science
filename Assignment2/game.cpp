...
//	}
	...
	if(lunchTime && !lunchTaken){
		c.r=c.a=255;
		c.g=c.b=0;
		this->menu.write(this->sdlr,"Lunch Time!!!!",c,4,32);
	}
...
//	cout<<endl;
	...
	//Render Player 2
	SDL_Rect srcp2={32+118*3,502+350,58,105};
	SDL_Rect absdstp2={100,200,32,32};
...
//	this->tid=SDL_AddTimer(3000,temp, (void*)("3 seconds waited!"));
	...
	this->timerFuncsInterval.insert({"startLunch",3*60*1000});
...
void Game::checkYulu(){
	if(checkLandmarkIntersection("YULU1", this->player1.dst)||checkLandmarkIntersection("YULU2", this->player1.dst)){
		if(!this->player1.onBike){
			SDL_SetRenderDrawBlendMode(this->sdlr, SDL_BLENDMODE_BLEND);
			SDL_SetRenderDrawColor(this->sdlr, 0,160,240, 100 );
			//SDL_RenderDrawRect(this->sdlr,&r );
			...
			//while(SDL_PollEvent(&event)){
				if( this->event.type==SDL_KEYUP && this->event.key.keysym.sym==SDLK_k){
	...
	if(event.key.keysym.sym==SDLK_t){
		...
		while(enter){
			...
			while(SDL_PollEvent(&event)){
				...
				if((!this->taskManager.taskOngoing && event.type==SDL_KEYDOWN &&event.key.keysym.sym == SDLK_s)){
					...
					if(this->event.key.keysym.sym==SDLK_e){
						...
						while(goals<=20){
							while(SDL_PollEvent(&event)){
								...
								if(((event.type==SDL_MOUSEBUTTONUP && event.button.button==SDL_BUTTON_LEFT )||(event.type==SDL_KEYUP &&event.key.keysym.sym == SDLK_s))){
