...
vector<vector<float>> read_matrix_mkl(string file){
	fstream stream;
	try {
		stream.open(file);
...
void write_matrix_mkl(string file, vector<vector<float>> matrix){
...
//float* matrix_to_array( vector<vector<float>> matrix,float* arr){
//	int c=matrix[0].size();
//	int r=matrix.size();
//	for(int i=0;i<c;i++){
//		for(int j=0;j<r;j++){
...
void write_vector_mkl(string file, vector<float> vec){
	ofstream stream(file);
	int n=vec.size();
	stream<<n<<'\n';
...
void print_matrix_mkl(vector<vector<float>> m){
...
void print_vector_mkl(vector<float> v){
...
vector<float> read_vector_mkl(string file){
	fstream stream;
	try {
			stream.open(file);
			//Catching error if input file is not present
...
vector<vector<float>> add_matrix_mkl(vector<vector<float>> a, vector<vector<float>> b){
...
vector<vector<float>> mul_matrix_mkl(vector<vector<float>> a, vector<vector<float>> b){
...
bool compare_matrices_mkl(vector<vector<float>> truth, vector<vector<float>> output){
	bool a=true;
...
vector<vector<float>> fc_output_mkl(string input, string weights,string bias){
	vector<vector<float>> i=read_matrix_mkl(input);
	vector<vector<float>> w=read_matrix_mkl(weights);
	vector<vector<float>> b=read_matrix_mkl(bias);
//	print_matrix(i);
//	cout<<'\n';
//	print_matrix(w);
//	cout<<'\n';
...
void create_plot_files_mkl(int matrix_size, int num_runs, string outputfile){
...
//			if(!compare_matrices_mkl(add_matrix_mkl(mul_matrix_mkl(input, weights), bias),output)){
//				cout<<"Wrong Answer \n";
//				//exit(0);
//			}
...
void create_all_plot_files_mkl(int num_runs){
	for(int i=10;i<=100;i+=10){
		create_plot_files_mkl(i, num_runs, "plot_mkl_"+to_string(i)+"_"+to_string(num_runs)+".txt");
	}
